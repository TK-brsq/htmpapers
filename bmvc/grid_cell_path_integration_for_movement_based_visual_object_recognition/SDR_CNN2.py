#SDR_CNN2.py
#for 4pixel x 4pixel = 7 x 7 patch
...
EPOCHS = 5  # Recommend 10
FIRST_EPOCH_BATCH_SIZE = 8  # Used for optimizing k-WTA, Default 4
TRAIN_BATCH_SIZE = 128  # Recommend 128
TEST_BATCH_SIZE = 512  # Default 1000
...
def test(model, loader, test_size, criterion, epoch, sdr_output_subset=None):
    ...
    plt.xlim(0, 0.6)
    plt.ylim(0, 70)
    plt.savefig("duty_cycle_results2/duty_cycle_boost_" + str(BOOST_STRENGTH) + "_"
                + str(epoch) + ".png")
    plt.clf()
    ...
    if sdr_output_subset is not None:
        print("Saving generated SDR and label outputs from data sub-section: "
              + sdr_output_subset)
        np.save("python2_htm_docker/docker_dir/training_and_testing_data2/" + DATASET
...
class SDRCNNBase(nn.Module):
    ...
    def __init__(self, percent_on, boost_strength):
        ...
        self.conv2 = nn.Conv2d(in_channels=64, out_channels=128, kernel_size=3,
                               padding=1) # [128, 14, 14]
        self.pool2 = nn.MaxPool2d(kernel_size=2, stride=2) # [128, 7, 7]
        self.k_winner = KWinners2d(channels=128, percent_on=percent_on,
                                   boost_strength=boost_strength, local=True)
        self.dense1 = nn.Linear(in_features=128 * 7 * 7, out_features=256)
...
if __name__ == "__main__":
    ...
    if os.path.exists("saved_networks2/") is False:
        try:
            os.mkdir("saved_networks2/")
        except OSError:
            pass

    if os.path.exists("duty_cycle_results2/") is False:
        try:
            os.mkdir("duty_cycle_results2/")
        except OSError:
            pass

    if (os.path.exists("python2_htm_docker/docker_dir/training_and_testing_data2/")
            is False):
        try:
            os.mkdir("python2_htm_docker/docker_dir/training_and_testing_data2/")
        except OSError:
            pass
    ...
    if TRAIN_NEW_NET is True:
        ...
        torch.save(sdr_cnn.state_dict(), "saved_networks2/sdr_cnn2.pt")

    else:
        print("Evaluating a pre-trained model:")
        sdr_cnn.load_state_dict(torch.load("saved_networks2/sdr_cnn2.pt"))
