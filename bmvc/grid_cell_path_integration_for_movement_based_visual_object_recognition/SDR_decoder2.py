...
class MLPDecoder(torch.nn.Module):
    def __init__(self):
        ...
        self.dense1 = nn.Linear(in_features=128 * 7 * 7, out_features=512)
...
def initialize():
    ...
    training_labels = torch.from_numpy(
        np.load("python2_htm_docker/docker_dir/training_and_testing_data2/"
...
def train_net(net, training_input, training_sources, training_labels):
    ...
    torch.save(net.state_dict(), "saved_networks2/" + DATASET + "_decoder2.pt")
...
def generate_images(net, net_input, sources, labels):
    ...
    net.load_state_dict(torch.load("saved_networks2/" + DATASET + "_decoder2.pt"))
    ...
    for batch_iter in range(1):
        ...
        for image_iter in range(len(batch_labels)):
            ...
            plt.imsave("decoder_reconstructed_images2/" + str(batch_iter) + "_"
...
if __name__ == "__main__":
    ...
    if os.path.exists("decoder_reconstructed_images2/") is False:
        try:
            os.mkdir("decoder_reconstructed_images2/")
        except OSError:
            pass
