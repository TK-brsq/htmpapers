                    ...
                    if not inferred:
                        ...
                        if inferred:
                            if np.argmax(class_node_activations) \
                                    == int(objectDescription["name"][0]):
                                #print("Correctly classified " + objectDescription["name"][0])
                                inferredStep = currentStep  # noqa: N806
                                plt.imsave("correctly_classified2/"
                                ...
                                if len(representation) == 40:
                                    ...
                                    #print("Converged to a single representation and"  " now inferred")
                                    single_converged = True
                                    single_converged_step = currentStep

                            else:
                                #print("Incorrectly classified a " + objectDescription["name"][0] + " as a " + str(np.argmax(class_node_activations)))
                                incorrect = {"never_converged": 0,
                                             "false_convergence": 1}
                                plt.imsave("misclassified2/example_"
                                           + objectDescription["name"]
                                           + "_converged_to_"
