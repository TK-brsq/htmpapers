...
"""
'''
result = {train, test, accuracy, false_coverge, sensation}

2025/05/11
{10, 20, 38.5%, 42.5%, 17.66}
train: 4.5min, test: 17min
{20, 20, 13.5 %, 8%, 27.66}
train: 14min, test: fail to measure
'''
import json
import math
import os
...
from GridCellNet_base2 import (
    PIUNCorticalColumnForVisualRecognition,
    PIUNExperimentForVisualRecognition,
)
from pre_process_SDRs2 import generate_image_objects
...
INPUT_GRID_DIMENSION = 7  # Options are 4 or 5, depending on the kind of pre-processing
# of the input used i.e. 7x7 pixel patches on a 4x4 grid, or 5x5 grid extracted from
# the middle layer of a CNN
NUM_TEST_OBJECTS_PER_CLASS = 20  # The number of test objects per class to use during
# evaluation
...
def object_learning_and_inference(
    ...
    #======================================================================================
    #               SETTING UP THE NETWORK, AND PREPARE TRAINING DATA
    #======================================================================================
    ...
    print("\nLearning objects")
    for learning_object_iter in tqdm(training_order_list):  # noqa: N806
    ...
    if EVAL_ON_TRAINING_DATA_BOOL:
        print("\nrecolling objects")
        for objectDescription in tqdm(test_objects):  # noqa: N806
        ...
        np.save("prediction_data2/object_prediction_sequences",
...
# path
def dir_setup():
    if os.path.exists("misclassified2/") is False:
        try:
            os.mkdir("misclassified2/")
        except OSError:
            pass

    if os.path.exists("correctly_classified2/") is False:
        try:
            os.mkdir("correctly_classified2/")
        except OSError:
            pass

    if os.path.exists("results2/") is False:
        try:
            os.mkdir("results2/")
        except OSError:
            pass

    if os.path.exists("prediction_data2/") is False:
        try:
            os.mkdir("prediction_data2/")
        except OSError:
            pass
            ...
            for class_threshold_iter in CLASS_THRESHOLDS_LIST:
                ...
                for num_flip_bits in FLIP_BITS_LIST:
                    ...
                    if not EVAL_ON_TRAINING_DATA_BOOL:
                        plt.savefig("results2/classification_visualization"
                    ...
                    with open("results2/GridCellNet_dendritic_threshold_"
                ...
                if EVAL_ON_TRAINING_DATA_BOOL:
                    ...
                    plt.ylim(0, 100)
                    plt.xlim(0, 130)
                    plt.savefig("results2/recall_vs_bitwise_noise__dendritic_threshold_"
